...
class BaseVehicleManager {
    ...
    // Parser actor to offload decoding from MainActor
                ...
                client.addPublishListener(named: "\(String(describing: Self.self))Listener") { [weak self] result in
                    ...
                    if case .success(let publishInfo) = result {
                        ...
                        Task {
                            ...
                            if let vehicle = await self.parser.parse(topicName: topic, payload: data, vehicleType: self.vehicleType) {
                                await MainActor.run {
                                    self.handleParsedVehicle(vehicle)
                                }
                            }
                        }
                    }
